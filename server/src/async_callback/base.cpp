...
GrpcAsyncCallbackServerBase::GrpcAsyncCallbackServerBase(const std::string& address) : server_address_(address) {}

void GrpcAsyncCallbackServerBase::Init() {
  InitServices();
...
bool GrpcAsyncCallbackServerBase::BuildAndStartServer() {
...
void GrpcAsyncCallbackServerBase::NonBlockRun() {
  Init();
...
void GrpcAsyncCallbackServerBase::Run() {
  Init();
  if (!BuildAndStartServer()) {
    std::cerr << "Server failed to start." << std::endl;
    return;
...
void GrpcAsyncCallbackServerBase::Shutdown() {
  if (server_) {
    server_->Shutdown();
